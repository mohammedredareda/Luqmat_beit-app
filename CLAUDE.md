...
- **One app, one shared package, Melos monorepo**: `apps/luqmat_beit_app`, depending on `packages/core` (path dependency). This supersedes the earlier two-separate-apps plan (`apps/customer_app` + `apps/cook_app`) — those were merged into a single app, role-routed between the customer and cook shells based on the logged-in account's role (see `SessionCubit` in `core` and `resolveRedirect` in `apps/luqmat_beit_app/lib/router/app_router.dart`). Role is fixed per account at registration; there is no in-app role switcher. Cook-only routes live under the `/cook` prefix; customer routes are unprefixed.
- **Clean Architecture, feature-first**: every feature folder is `data/` (datasources, models, repository impls) / `domain/` (repository interfaces, usecases) / `presentation/` (bloc, pages, widgets). `packages/core` has zero knowledge of the app and zero Flutter/UI imports in its domain layer.
...
- **Routing**: `go_router`, a single router for the whole app, `redirect` gated on `SessionCubit`'s state (auth + role) via `resolveRedirect` — not just whether there's a session, but which role it belongs to, since an authenticated cook and an authenticated customer see disjoint route trees.
...
This is the source of truth for what each screen should look like — **use these instead of designing screens from scratch.** It's a Stitch export covering **the cook role only** (screens reachable under the `/cook` route prefix in `apps/luqmat_beit_app`, plus shared flows like onboarding/auth/OTP/splash that the customer role also uses). Each screen folder contains:
