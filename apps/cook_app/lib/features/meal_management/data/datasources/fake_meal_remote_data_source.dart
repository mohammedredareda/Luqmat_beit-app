...
class FakeMealRemoteDataSource {
  ...
  static const List<MealModel> _handCraftedSeed = [
        ...
        // Individually Stopped (CK-24) — exercises the "independently
        // inactive, unaffected by kitchen-wide resume" scenario.
        MealModel(
          id: 'meal-3',
          cookId: currentCookId,
          name: 'شوربة عدس',
  ...
  /// Meals belonging to [cookId], excluding soft-deleted ones (CK-09).
  /// `isActive` is always derived by the caller via
  /// `MealEntity.computeIsActive` — this never bakes [isStopped] or the
  /// kitchen-wide pause into a stored flag, so re-enabling selling can
  /// never accidentally resurrect an independently-Stopped meal (CK-10).
