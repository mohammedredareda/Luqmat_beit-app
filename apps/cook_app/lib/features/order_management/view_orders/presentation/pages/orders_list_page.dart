...
import '../../../shared/presentation/order_status_colors_x.dart';
...
class _EndOfListMarker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final label = switch (status) {
      ...
      OrderStatus.done => l10n.endOfListCompletedLabel,
      OrderStatus.rejected || OrderStatus.cancelled => l10n.endOfListRejectedLabel,
    };
...
class _EmptyBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    final statusColors = Theme.of(context).extension<OrderStatusColors>()!;
    final status = currentFilterStatus;
    final (background, foreground) = status.colorsOf(statusColors);
    ...
    final (heading, body) = switch (status) {
      ...
      OrderStatus.done => (l10n.emptyOrdersDoneHeading, l10n.emptyOrdersDoneBody),
...
/// List-context status label — the mockup's pending-order badge reads
/// "طلب جديد" (matching the "New" filter's own copy), distinct from
/// `orderStatusPending`'s "قيد الانتظار" used on the Order Details screen.
/// Other statuses reuse the shared detail-screen labels (no evidence they
/// should differ in list context).
String _cardStatusLabel(AppLocalizations l10n, OrderStatus status) => switch (status) {
      OrderStatus.pending => l10n.orderCardNewBadge,
      OrderStatus.preparing => l10n.orderStatusPreparing,
      OrderStatus.done => l10n.orderStatusDone,
