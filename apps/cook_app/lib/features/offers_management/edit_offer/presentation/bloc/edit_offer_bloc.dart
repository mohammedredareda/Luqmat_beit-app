...
class EditOfferBloc extends Bloc<EditOfferEvent, EditOfferState> {
  ...
  Future<void> _load(String offerId, Emitter<EditOfferState> emit) async {
    ...
    result.fold(
      (bundle) => emit(EditOfferState.form(EditOfferFormData(
        offerId: bundle.offer.id,
        name: bundle.offer.name,
        description: bundle.offer.description,
        totalPriceInput: bundle.offer.totalPrice.toString(),
        durationDaysInput: bundle.offer.durationDays.toString(),
        isActive: bundle.offer.isActive,
        imagePath: bundle.imageUrl,
        includedMeals: [
          for (final meal in bundle.mealDrafts)
            OfferMealDraft(
              mealId: meal.mealId,
