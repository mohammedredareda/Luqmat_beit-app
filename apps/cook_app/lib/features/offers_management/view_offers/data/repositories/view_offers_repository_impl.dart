...
class ViewOffersRepositoryImpl implements ViewOffersRepository {
  ...
  Future<Result<PaginatedResult<OfferFeedItemEntity>>> getOffersFeed(
    ...
    return guard(() async {
      ...
      final feed = <OfferFeedItemEntity>[
        for (final model in offerModels)
          OfferFeedOfferItem(model.toEntity(), imageUrl: model.imageUrl),
        for (final model in discountModels)
          OfferFeedDiscountItem(
            model.toEntity(),
            mealName: model.mealName,
            mealImageUrl: model.mealImageUrl,
            mealBasePrice: model.mealBasePrice,
          ),
  ...
  bool _matchesFilter(OfferFeedItemEntity item, OfferFeedFilter filter) {
    ...
    final expired = switch (item) {
      OfferFeedOfferItem(:final offer) =>
        offer.expiryTime.difference(DateTime.now()).inDays <= 0,
      OfferFeedDiscountItem(:final discount) =>
        discount.expiryTime.difference(DateTime.now()).inDays <= 0 ||
