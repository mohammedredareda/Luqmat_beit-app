...
class DiscountCard extends StatelessWidget {
  const DiscountCard({super.key, required this.discount, this.onTap, this.onEdit, this.onDelete});
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = expired
        ...
        : discount.remainingDays != null
            ? l10n.offerExpiresInLabel(discount.remainingDays!)
            : l10n.remainingUsageValue(discount.remainingUsage ?? 0);

    return _OfferDiscountCardShell(
      onTap: onTap,
      header: _PhotoHeader(imageUrl: discount.mealImageUrl),
      ...
      badge: _Badge(
        icon: Icons.percent,
        label: l10n.discountBadgeLabel(discount.percentage.toStringAsFixed(0)),
        backgroundColor: scheme.error,
        foregroundColor: scheme.onError,
      ),
      title: discount.mealName,
      chip: subtitle,
      priceRow: Row(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          Text(discount.mealBasePrice.toStringAsFixed(0), style: AppText.priceOld(context)),
          const SizedBox(width: AppSpace.xs),
          Text(discount.discountedPrice.toStringAsFixed(0), style: AppText.price(context)),
        ],
      ),
      onEdit: onEdit,
      onDelete: onDelete,
...
class OfferCard extends StatelessWidget {
  const OfferCard({super.key, required this.offer, this.onTap, this.onEdit, this.onDelete});
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    final scheme = Theme.of(context).colorScheme;
    final expired = offer.remainingDays <= 0;

    final hasImage = offer.imageUrl != null && offer.imageUrl!.isNotEmpty;

    return _OfferDiscountCardShell(
      onTap: onTap,
      header: hasImage ? _PhotoHeader(imageUrl: offer.imageUrl!) : const _BundlePlaceholderHeader(),
      badge: _Badge(
        icon: Icons.sell,
      ...
      title: offer.name,
      chip: expired ? l10n.offerExpiredLabel : l10n.offerExpiresInLabel(offer.remainingDays),
      description: [
        for (final meal in offer.includedMeals) '${meal.quantity} ${meal.mealName}',
      ].join(' + '),
