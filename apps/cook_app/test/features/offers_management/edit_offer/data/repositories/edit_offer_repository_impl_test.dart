...
void main() {
  ...
  test('getOffer on the seeded offer-1 returns its data', () async {
    ...
    expect(offer.includedMeals, hasLength(3));
  });
  ...
  test('getOffer on an unknown id returns Result.failure(NotFoundException)', () async {
    ...
    expect(result, isA<Failure<OfferEntity>>());
    expect((result as Failure<OfferEntity>).exception, isA<NotFoundException>());
  });
  ...
  test('updateOffer mutates and a subsequent getMyOffers reflects it', () async {
    ...
    final myOffers = await dataSource.getMyOffers(updated.cookId);
